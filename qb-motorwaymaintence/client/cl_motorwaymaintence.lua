    ...
    if not DoesEntityExist(conBoss) then
        ...
        exports['qb-target']:AddTargetEntity(conBoss, { 
            options = {
                { 
                    type = "client",
                    event = "qb-motorwaymaintence:client:finishJob",
                ...
                { 
                    type = "client",
                    event = "qb-motorwaymaintence:client:startJob",
...
function AssignTask()
    ...
    exports['qb-target']:AddCircleZone("workBox", vector3(newtask.x, newtask.y, newtask.z), 1.3,{ name = "workBox", debugPoly = false, useZ=true, }, { options = { { type = "client", event = "qb-motorwaymaintence:client:completeRepairs", icon = "fa-solid fa-hammer", label = "Repair", job = "motorwaymaintence" }, }, distance = 1.5 })
    activeJob = true
    QBCore.Functions.Notify("You have been assigned a new task.", "success")
end

RegisterNetEvent('qb-motorwaymaintence:client:startJob', function(vehicle)
    local vehicle = Config.JobVehicle
    local coords = Config.VehicleSpawn
    if Config.ServerSideSpawn then -- Newer Core.
        TriggerServerEvent("qb-motorwaymaintence:server:SetJob")
        QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
            local veh = NetToVeh(netId)
            SetVehicleNumberPlateText(veh, "motorway_maintence"..tostring(math.random(1000, 9999)))
            DecorSetFloat(veh, "qb-motorwaymaintence", 1)
...
RegisterNetEvent('qb-motorwaymaintence:client:finishJob', function()
...
RegisterNetEvent('qb-motorwaymaintence:client:completeRepairs', function()
    if activeJob then
        ...
        QBCore.Functions.Progressbar("motorway_maintence", "Completing the task..", Config.ProgressBarTime, false, false, {
            ...
            TriggerServerEvent("qb-motorwaymaintence:server:taskPayout", newtask)
